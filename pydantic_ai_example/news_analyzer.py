...
class NewsTopics(BaseModel):
    ...
    secondary_topics: List[str] = Field(description="Additional topics mentioned", max_items=3)
...
def search_and_analyze():
    ...
    for term in search_terms:
        ...
        if articles:
            ...
            if articles:
                article_text = f"Title: {articles[0].title}\nSummary: {articles[0].summary}"
                try:
                    sentiment_result = sentiment_agent.run_sync(
                        f"Analyze sentiment: {article_text}",
                        deps=db
                    )
